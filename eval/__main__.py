...
def main() -> None:
    ...
    ap.add_argument("--run", action="store_true", help="Run scenarios in text mode (needs LLM key + DB)")
    ...
    if not args.run:
        ...
        for s in scenarios:
            print(f"  - {s.id:<22} vertical={s.vertical or '-':<13} expect={s.expected.outcome or '-'}")
